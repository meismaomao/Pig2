...
def main(argv=None):
	...
	clas1_acc = tf.reduce_mean(tf.cast(tf.equal(tf.argmax(train_labels, 1), tf.argmax(logits, 1)), tf.float32),
	                           name='clas1_acc')
	...
	with tf.Session() as sess:
		...
		for step in range(NUM//BATCH_SIZE):
			...
			Clas1_loss = sess.run(classifier1_loss, feed_dict=feed_dict)
